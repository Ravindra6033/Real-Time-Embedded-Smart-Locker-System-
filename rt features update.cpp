        ...
        if (state == State::IDLE) {
            if (hasItem) {
                ...
                if (itemDetectCount >= 10) { 
                    ...
                    itemDetectCount = 0; 
                    std::cout << "Item detected: Transition to SET_PASSWORD\n"; 
                }
            } else {
                itemDetectCount = 0;
            }
        } 
        // Logic to detect if the item was removed before setting the PIN
        else if (state == State::SET_PASSWORD) {
            if (!hasItem) {
                itemDetectCount--; // Debounce removal
                if (itemDetectCount <= -5) { 
                    state = State::IDLE;
                    itemDetectCount = 0;
                    pinMsg = "";
                    std::cout << "Item removed: Resetting to IDLE\n";
                }
            } else {
                itemDetectCount = 0; 
            }
...
int main() {
    ...
    std::cout << "=========================================\n";
    std::cout << "Smart Locker — PIN Protected\n";
